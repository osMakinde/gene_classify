######Implementation of DD4 for colon cancer data####
...
######Implementation of DD4 for Leukemia cancer data####
mrank<-function(x,xdata){
  n<-dim(xdata)[1]
  d<-dim(xdata)[2]
  s1<-0
...
######Implementation of DD4 for lymphoma cancer data####
mrank<-function(x,xdata){
  n<-dim(xdata)[1]
  d<-dim(xdata)[2]
  s1<-0
...
######Implementation of DD4 for postrate cancer data####
mrank<-function(x,xdata){
  n<-dim(xdata)[1]
  d<-dim(xdata)[2]
  s1<-0
...
######Implementation of DD4 for SRBCT cancer data####
mrank<-function(x,xdata){
  n<-dim(xdata)[1]
  d<-dim(xdata)[2]
  s1<-0
...
corprob<-NULL
for(k in 1:1000){            
  S1<-sample(n1,m1, replace=FALSE);  S2<-sample(n2,m2, replace=FALSE)
  S3<-sample(n3,m3, replace=FALSE);  S4<-sample(n4,m4, replace=FALSE)
  x1<-x1a[S1,]
  x2<-x2a[S2,]
  x3<-x3a[S3,]
  x4<-x4a[S4,]
  ...
  miscount<-0
  for(i in 1:14){
    rx<-mrank(z1[i,],x1);    ry<-mrank(z1[i,],x2);    rz<-mrank(z1[i,],x3);    rw<-mrank(z1[i,],x4)
    Fx<-sum(r1<rx)/m1;    Fy<-sum(r2<ry)/m2;    Fz<-sum(r3<rz)/m3;    Fw<-sum(r4<rw)/m4
    if(Fx<Fy && Fx<Fz && Fx<Fw) rightcount<-rightcount+1
  }
  z2<-x2a[-S2,]
  for(i in 1:4){
    rx<-mrank(z2[i,],x1);    ry<-mrank(z2[i,],x2);    rz<-mrank(z2[i,],x3);    rw<-mrank(z2[i,],x4)
    Fx<-sum(r1<rx)/m1;    Fy<-sum(r2<ry)/m2;    Fz<-sum(r3<rz)/m3;    Fw<-sum(r4<rw)/m4
    if(Fy<Fx && Fy<Fz && Fy<Fw) rightcount<-rightcount+1
  }
  z3<-x3a[-S3,]
  for(i in 1:9){
    rx<-mrank(z3[i,],x1);    ry<-mrank(z3[i,],x2);    rz<-mrank(z3[i,],x3);    rw<-mrank(z3[i,],x4)
    Fx<-sum(r1<rx)/m1; Fy<-sum(r2<ry)/m2; Fz<-sum(r3<rz)/m3; Fw<-sum(r4<rw)/m4
    if(Fz<Fx && Fz<Fy && Fz<Fw) rightcount<-rightcount+1
  }
  z4<-x4a[-S4,]
  for(i in 1:10){
    rx<-mrank(z4[i,],x1);    ry<-mrank(z4[i,],x2);    rz<-mrank(z4[i,],x3);    rw<-mrank(z4[i,],x4)
    Fx<-sum(r1<rx)/m1;    Fy<-sum(r2<ry)/m2;    Fz<-sum(r3<rz)/m3;    Fw<-sum(r4<rw)/m4
