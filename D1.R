######Implementation of D1 for colon cancer data####
...
######Implementation of D1 for Leukemia cancer data####
...
######Implementation of D1 for lymphoma cancer data####
mrank<-function(x,xdata){
  n<-dim(xdata)[1]
  d<-dim(xdata)[2]
  s1<-0
...
corprob<-NULL
for(k in 1:1000){            
  S1<-sample(n1,m1, replace=FALSE); S2<-sample(n2,m2, replace=FALSE); S3<-sample(n3,m3, replace=FALSE)
  x1<-x1a[S1,]
  x2<-x2a[S2,]
  x3<-x3a[S3,]
  rightcount<-0
...
######Implementation of D1 for postrate cancer data####
mrank<-function(x,xdata){
  n<-dim(xdata)[1]
  d<-dim(xdata)[2]
  s1<-0
...
corprob<-NULL
for(k in 1:1000){            
  S1<-sample(n1,m, replace=FALSE); S2<-sample(n2,m, replace=FALSE)
  x1<-x1a[S1,]
  x2<-x2a[S2,]
  rightcount<-0
  z<-x1a[-S1,]
  miscount<-0
  for(i in 1:20){
    rx<-mrank(z[i,],x1); ry<-mrank(z[i,],x2)
    if(rx<ry) rightcount<-rightcount+1
  }
  z<-x2a[-S2,]
  for(i in 1:22){
...
######Implementation of D1 for SRBCT cancer data####
mrank<-function(x,xdata){
  n<-dim(xdata)[1]
  d<-dim(xdata)[2]
  s1<-0
