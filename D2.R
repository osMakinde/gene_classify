######Implementation of D2 for colon cancer data####
mrank<-function(x,xdata){
  temp<-x-xdata
  normtemp<-(sum((temp)^2))^0.5
}
...
######Implementation of D2 for Leukemia cancer data####
...
for(k in 1:1000){            
  ...
  x<-x1a[S1,]
  y<-x2a[S2,]
  xm<-apply(x, 2, mean);   ym<-apply(y, 2, mean)
  rightcount<-0
  z<-x1a[-S1,]
  miscount<-0
  for(i in 1:12){
...
######Implementation of D2 for lymphoma cancer data####
mrank<-function(x,xdata){
  temp<-x-xdata
  normtemp<-(sum((temp)^2))^0.5
}
...
for(k in 1:1000){            
  ...
  x2<-x2a[S2,]
  x3<-x3a[S3,]
  xm<-apply(x1,2,mean); ym<-apply(x2,2,mean); zm<-apply(x3,2,mean)
  rightcount<-0
  z1<-x1a[-S1,]
  for(i in 1:12){
...
######Implementation of D2 for postrate cancer data####
mrank<-function(x,xdata){
  temp<-x-xdata
  normtemp<-(sum((temp)^2))^0.5
}
...
for(k in 1:1000){            
  ...
  x1<-x1a[S1,]
  x2<-x2a[S2,]
  xm<-apply(x1, 2, mean);  ym<-apply(x2, 2, mean)
  rightcount<-0
  z1<-x1a[-S1,]
  for(i in 1:20){
...
######Implementation of D2 for SRBCT cancer data####
mrank<-function(x,xdata){
  temp<-x-xdata
  normtemp<-(sum((temp)^2))^0.5
}
